""" 
There are four changes in this file that are not present in the original `script_3_fine_tuning` in Graph-BERT folder. 
...
if not os.path.exists(node_path):
    raise FileNotFoundError(
        ...
        f"  or: ./run.sh <cores> <gpu> script_3_fine_tuning.py {dataset_name} {embedder}"
    )
...
# ---- Fine-Tuning: Graph Bert Node Classification ----
if 1:
    # ---- hyper-parameters ----
    lr = 0.001
    k = 7
 
    # Per-dataset max_epoch (matching the original script's values)
    EPOCH_MAP = {
        'ppi': 250,
        'human': 120,
        'c.elegan': 120,
    ...
    max_epoch = EPOCH_MAP.get(dataset_name, 120)
    ...
    print(f'GraphBert | dataset: {dataset_name} | embedder: {embedder} | '
          f'residual: {residual_type} | k: {k} | '
          f'hidden: {hidden_size} | layers: {num_hidden_layers} | '
          f'heads: {num_attention_heads} | x_size: {x_size}')
    ...
    result_obj.result_destination_file_name = (
        f'{dataset_name}_{embedder}_{num_hidden_layers}'
    )
    ...
    # Save the fine-tuned model (tagged with embedder)
    save_dir = (
        f'./result/PreTrained_GraphBert/{dataset_name}_{embedder}/'
        f'node_classification_complete_model/'
    )
    method_obj.save_pretrained(save_dir)
